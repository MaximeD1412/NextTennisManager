...
- **Fixed parameters (simulator contract)** — *which Attributs exist* and their scale (1–99) are fixed across all Mondes. Dynamic states (Rythme, Fatigue, Moral) are always transmitted to the simulator as normalised coefficients (0.0–1.0), regardless of how they are stored in the WebApp. The simulator is built against this interface. MondeSetting cannot add, remove, or rescale Attributs, nor change the coefficient contract for dynamic states. Note: ELO was considered as a simulator input but rejected as redundant — Attributs already fully encode player strength. Prestige exists as a meta-game layer and is not a simulator input.
